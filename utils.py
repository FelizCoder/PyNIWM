...
def create_folds_with_SMOTE(X_cv, y_cv, n_splits, smoter=None):
    ...
    for fold_, (tra_, val_) in enumerate(skfold.split(X_cv, y_cv)):
        ...
        folds.append(
            {
                "train": (X_train, y_train, tra_),
                "valid": (X_valid, y_valid, val_),
                "scaler": scaler,
            }
        )
    return folds
...
def single_model_cv(folds, algorithm, params, n_valid, X_test=None, disable_tdqm=False):
    ...
    for i, fold in enumerate(tqdm(folds, disable=disable_tdqm)):
        ...
        X_valid, y_valid, val_ = fold[
            "valid"
        ]  # retrieve also validation indexes for oof_preds
        # build model
...
def best_model_selection(configfile, num_seed):
    ...
    for j in range(num_seed[0]):
        # results = pd.read_pickle('cv_results/cv_results_seed{}'.format(j))
        results = pd.read_pickle(
            configfile["cvResultsDir"] + "/cv_results_seed{}".format(j)
        )
        performance_distribution = pd.concat(
            [performance_distribution, results], ignore_index=True
        )
    # final selection of best params combination
    final_models = pd.DataFrame(
        columns=["algorithm", "params", "median_f1score", "time"]
    )
...
def single_model_cv_and_test(folds, alg_name, params, X_test, maj_vote="hard"):
    ...
    for fold in folds:
        n_valid += fold["valid"][0].shape[0]
    f1scores, oof_preds, fold_tst_preds = single_model_cv(
        folds, algorithm, params, n_valid, X_test, True
    )
    # majority vote
...
def get_confusion_matrix(y_test, y_hat, plot=True, labels=None):

    # compute normalized confusion matrix
    cmNorm = confusion_matrix(y_test, y_hat, labels=labels, normalize="true")
    ...
    if plot:
        ...
        heatmap = sns.heatmap(
            cmNorm[:, count_sort_ind][count_sort_ind],
            annot=True,
            fmt=".0%",
            cmap="Blues",
            ax=ax,
        ...
        heatmap.yaxis.set_ticklabels(
            fixtures[count_sort_ind], rotation=0, ha="right", fontsize=12
        )
        heatmap.xaxis.set_ticklabels(
            fixtures[count_sort_ind], rotation=45, ha="right", fontsize=12
        )
